...
export default function TripDetail() {
  ...
  const handleCreateShare = async () => {
    ...
    try {
      await tripSharingApi.shareWith(tripId!, shareEmail);
      setShareEmail("");
      const { data } = await tripSharingApi.getShares(tripId!);
      setExistingShares(data.shares);
      toast.success("分享成功");
      ...
      <Dialog open={showAddActivity} onOpenChange={setShowAddActivity}>
        <DialogContent className="bg-white sm:max-w-lg rounded-[32px] p-0 overflow-hidden border-none shadow-2xl">
          ...
          <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
            ...
            <div className="space-y-1.5">
              ...
              <Input 
                ...
                className="h-12 rounded-xl border-[oklch(0.88_0.008_220)] focus:border-[oklch(0.62_0.12_220)]" 
              />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label className="text-sm font-medium text-[oklch(0.35_0.06_220)]">開始時間</Label>
                <Input 
                  type="time" 
                  value={form.time} 
              ...
              <div className="space-y-1.5">
                <Label className="text-sm font-medium text-[oklch(0.35_0.06_220)]">預計時長 (分鐘)</Label>
                <Input 
                  type="number" 
                  placeholder="60" 
                  value={form.duration} 
            ...
            <div className="space-y-1.5">
              ...
              <PlaceSearch 
                onPlaceSelect={(place) => setForm({ ...form, location: place.name, address: place.address, lat: place.lat, lng: place.lng })} 
                placeholder="搜尋 Google 地點..." 
                defaultValue={form.location} 
              />
            </div>
            <div className="space-y-1.5">
              <Label className="text-sm font-medium text-[oklch(0.35_0.06_220)]">預估花費 ({trip.currency})</Label>
              <Input 
                type="number" 
                placeholder="0" 
                value={form.cost} 
          ...
          <div className="p-8 pt-0 flex gap-3">
            <Button variant="outline" onClick={() => setShowAddActivity(false)} className="flex-1 h-12 rounded-xl border-[oklch(0.88_0.008_220)]">取消</Button>
            <Button 
              onClick={editingActivity ? handleUpdateActivity : handleCreateActivity} 
              disabled={isSaving || !form.title} 
              className="flex-1 h-12 rounded-xl bg-[oklch(0.22_0.08_220)] hover:bg-[oklch(0.35_0.06_220)] text-white"
            >
              {isSaving ? "儲存中..." : (editingActivity ? "更新活動" : "新增活動")}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={!!deletingActivity} onOpenChange={() => setDeletingActivity(null)}>
        <AlertDialogContent className="rounded-[32px] bg-white border-none p-8">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-gray-500">「{deletingActivity?.title}」將被永久移除。</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-6 gap-3">
            <AlertDialogCancel className="flex-1 h-12 rounded-xl border-[oklch(0.88_0.008_220)]">取消</AlertDialogCancel>
            <AlertDialogAction onClick={handleDeleteActivity} className="flex-1 h-12 rounded-xl bg-red-500 hover:bg-red-600 text-white border-none">確定刪除</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
